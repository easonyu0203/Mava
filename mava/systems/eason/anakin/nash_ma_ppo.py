...
from mava.systems.eason.types import LearnerState, OptStates, Params, PPOTransition
...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(
            ...
            params, magnet_param, opt_states, key, env_state, last_timestep, last_done = learner_state
            ...
            learner_state = LearnerState(params, magnet_param, opt_states, key, env_state, timestep, done)
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    magnet_policy = actor_apply_fn(magnet_param, traj_batch.obs)
                    ...
                    magnet_div = actor_policy.kl_divergence(magnet_policy).mean()

                    total_actor_loss = (
                        actor_loss
                        - config.system.ent_coef * entropy
                        + config.system.magnet_coef * magnet_div
                    )
                    return total_actor_loss, (actor_loss, entropy, magnet_div)
                ...
                actor_loss, (_, entropy, magnet_div) = actor_loss_info
        ...
        params, opt_states, traj_batch, advantages, targets, key = update_state
        learner_state = LearnerState(params, magnet_param, opt_states, key, env_state, last_timestep, last_done)
...
def learner_setup(
    ...
    replicate_learner = (params, actor_params.copy(), opt_states, step_keys, dones)
    ...
    params, magnet_param, opt_states, step_keys, dones = replicate_learner
    init_learner_state = LearnerState(params, magnet_param, opt_states, step_keys, env_states, timesteps, dones)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    assert (
        config.system.num_update_per_magnet % config.system.num_updates_per_eval == 0
    ), "Number of updates per magnet must be divisibile by number of updates per evaluation."
    ...
    for eval_step in range(config.arch.num_evaluation):
        ...
        num_param_updates = (eval_step + 1) * config.system.num_updates_per_eval
        episode_metrics, ep_completed = get_final_step_metrics(learner_output.episode_metrics)
        episode_metrics["steps_per_second"] = steps_per_rollout / elapsed_time

        # Separately log timesteps, actoring metrics and training metrics.
        logger.log({"num_param_updates": num_param_updates}, t, eval_step, LogEvent.MISC)
        ...
        # update magnet
        if (eval_step + 1) * config.system.num_updates_per_eval % config.system.num_update_per_magnet == 0:
            print(f"{Fore.YELLOW}{Style.BRIGHT}Updating magnet{Style.RESET_ALL}")
            learner_state = learner_state._replace(magnet_param=learner_state.params.actor_params.copy())
